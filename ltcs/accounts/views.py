...
from accounts.functions import ApiRes
...
class TokenView(TokenViewBase):
    ...
    def post(self, request, *args, **kwargs):
        self._serializer_class = 'accounts.serializers.CreateTokenExtend'
    ...
    def put(self, request, *args, **kwargs):
        self._serializer_class = 'accounts.serializers.RenewToken'
        return super().post(request, *args, **kwargs)
    ...
    def create(self, request):
        ...
        try:
            with transaction.atomic():
                register_user = User.objects.create_user(username=username, email=email, password=password)

                user_info = {
                    "id":register_user.id
                }

            return ApiRes().set_success_msg().generate(status_code=200,return_data=user_info)
